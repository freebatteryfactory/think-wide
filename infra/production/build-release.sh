...
python3 - "${VITE_CONVEX_URL:-}" <<'VALIDATE'
import sys
...
python3 - "$release_output" "$release_commit" "$release_image" "${VITE_CONVEX_URL:-}" <<'PY'
...
(out / 'release.json').write_text(json.dumps({
    ...
    'browserConvexUrl': sys.argv[4] or None, 'publicActivation': 'NOT_RUN',
}, indent=2) + '\n')
PY
