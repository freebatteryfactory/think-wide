...
export async function currentCatalogGrant(ctx: QueryCtx, grant: Doc<"grants">) {
	...
	const snapshot = await ctx.db
		.query("snapshots")
		.withIndex("by_snapshot", (q) => q.eq("snapshotId", grant.resourceId))
		.unique();
	return (
		!!snapshot?.demoCatalog && snapshot.catalogEpoch === grant.catalogEpoch
	);
}
...
export async function claimCatalog(ctx: MutationCtx, principal: Principal) {
	const snapshots = await ctx.db
		.query("snapshots")
		.withIndex("by_catalog", (q) => q.eq("demoCatalog", true))
		.take(9);
	if (!snapshots.length)
		return fail("capability_disabled", "Demo catalog is unavailable");
	if (snapshots.length > 8)
		return fail(
			"limit_exceeded",
			"Demo catalog exceeds the eight-snapshot limit",
		);
	for (const snapshot of snapshots) {
		const project = decode<Project>(validators.Project, snapshot.project);
		if (project.dataLabel !== "public" || snapshot.catalogEpoch === undefined)
			return fail("internal", "Invalid catalog configuration");
		const grants = await ctx.db
			...
			.withIndex("by_principal_resource", (q) =>
				q
					...
					.eq("resourceId", snapshot.snapshotId),
			)
			.collect();
		...
		if (
			grants.some(
				(grant) =>
					...
					(grant.catalogEpoch !== undefined &&
						grant.catalogEpoch !== snapshot.catalogEpoch),
			)
		)
	...
	return ctx.db.insert("catalogClaims", {
		...
		snapshots: snapshots.map((snapshot) => ({
			...
			epoch: snapshot.catalogEpoch ?? 0,
		})),
	});
}

export async function readCatalogClaim(
	ctx: QueryCtx,
	...
	for (const item of claim.snapshots) {
		...
		if (!snapshot.demoCatalog || snapshot.catalogEpoch !== item.epoch)
			return fail("not_found", "Resource not found");
		const project = decode<Project>(validators.Project, snapshot.project);
...
export async function setCatalog(
	...
	args: {
		...
		input: { snapshotId: string; enabled: boolean };
	},
) {
	...
	if (
		...
		!validators.ReadHistoryRequest({ snapshotId: input.snapshotId })
	)
		return fail("invalid_request", "Invalid catalog configuration");
	const grants = await ctx.db
		.query("grants")
		.withIndex("by_principal_resource", (q) =>
			q
				.eq("principal", owner.id)
				.eq("resourceKind", "snapshot")
				.eq("resourceId", input.snapshotId),
		)
		.collect();
	if (
		!grants.some(
	...
	const snapshot = await ctx.db
		.query("snapshots")
		.withIndex("by_snapshot", (q) => q.eq("snapshotId", input.snapshotId))
		.unique();
	if (!snapshot) return fail("not_found", "Resource not found");
	const project = decode<Project>(validators.Project, snapshot.project);
	if (input.enabled && project.dataLabel !== "public")
		return fail(
			"invalid_request",
			"Only public source snapshots may enter the demo catalog",
		);
	...
	if (!Number.isSafeInteger(epoch))
		return fail("limit_exceeded", "Catalog epoch exhausted");
	await ctx.db.patch(snapshot._id, {
		demoCatalog: input.enabled,
		catalogEpoch: epoch,
	});
	return null;
}
...
export async function catalogIndexedAt(
	ctx: QueryCtx,
	args: { ownerTokenIdentifier: string; input: { snapshotId: string } },
) {
	const owner = await provisioningRecipient(ctx, args);
	if (
		!args.input ||
		Object.keys(args.input).length !== 1 ||
		!validators.ReadHistoryRequest({ snapshotId: args.input.snapshotId })
	)
		return fail("invalid_request", "Invalid snapshot selection");
	const snapshot = await ctx.db
		.query("snapshots")
		.withIndex("by_snapshot", (q) => q.eq("snapshotId", args.input.snapshotId))
		.unique();
	...
	const grants = await ctx.db
		...
		.withIndex("by_principal_resource", (q) =>
			q
				.eq("principal", owner.id)
				.eq("resourceKind", "snapshot")
				.eq("resourceId", args.input.snapshotId),
		)
		.collect();
	if (
		!grants.some(
